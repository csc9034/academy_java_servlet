...
<body>
	...
	4. application : 하나의 웹 어플리케이션 당 1개씩만 생성되는 객체
					웹 컨테이너에 하나의 서비스가 배포되어서 유지되는 동안
					계속하여 유지된다.
					서버가 종료될 때가지 하나의 객체가 유지됨.
					즉, 유지 범위가 가장 길다.
	...
	* 내장객체의 유지 범위
	...
	<%
		// 1. jsp 내장 객체 중 page에 attribute 로 id 를 추가
		pageContext.setAttribute("id", "gildong-page");
		
		// 2. jsp 내장 객체 중 request 에 attribute 로 id 를 추가
		request.setAttribute("id", "gildong-request");
		request.setAttribute("name", "길동이-req");
		
		// 3. jsp 내장 객체 중 session 에 attribute 로 id 를 추가
		session.setAttribute("id", "gildong-session");
		session.setAttribute("name", "길동이-session");
		
		// 4. jsp 내장 객체 중 application 에 attribute로 id 를 추가
	...
	5. 범위를 명시하지 않고 속성 값만 명시할 때 어떤 범위만 선택되는가
	 => 가장 작은 범위의 것이 자동 선택됨
	 \${id } = ${id }
	 \${name } = ${name }
	</pre>

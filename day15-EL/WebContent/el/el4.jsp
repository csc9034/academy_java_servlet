...
<%
	...
	java.util.List<String> stars = new ArrayList<>();
...
<h3> EL 에서 내장객체인 request 에 추가된 객체를 추출</h3>
...
3. \${stars[2] } = ${stars[2] }
	requsetScope을 생략 가능. 다른 내장객체에 동일한 이름(stars)가
	attribute로 추가 된 것이 없을 때만
4. \${stars["3"] } = ${stars["3"] }
...
1. \${requestScope.starMap["S2"] } = ${requestScope.starMap["S2"] }
1. \${starMap[S3] } = ${starMap[S3] }
1. \${starMap["S4"] } = ${starMap["S4"] }
1. \${starMap["S5"] } = ${starMap["S5"] }

</pre>
<hr/>
<h3>scriptlet, expression Tag로 목록 추출시 (List)</h3>
<%
	// request 객체에서 attribute 를 추출 (getAttribute() 메소드 사용)
...
<h3>scriptlet, expression Tag로 목록 추출시 (Map)</h3>
<%

	// request 객체에서 attribute 를 추출
...
1. &lt;%=starNameMap.get("S2") %&gt; = <%=starNameMap.get("S2") %> <br/>
1. &lt;%=starNameMap.get("S3") %&gt; = <%=starNameMap.get("S3") %> <br/>
1. &lt;%=starNameMap.get("S4") %&gt; = <%=starNameMap.get("S4") %> <br/>
1. &lt;%=starNameMap.get("S5") %&gt; = <%=starNameMap.get("S5") %> <br/>

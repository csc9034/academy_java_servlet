...
<pre>
EL 의 empty 연산자 : 존재하는 여부를 검사하는 연산자, 없을 때 true

1. parameter name (가)이 존재하는가? \${not empty param.name } = ${not empty param.name } 

2. parameter address (가)이 존재하는가? \${not empty param.address } = ${not empty param.address }  

3. parameter id (가)이 존재하는가? \${not empty param.id } = ${not empty param.id } 

4. parameter name (가)이 없는가? \${not empty param.name } = ${ empty param.name } 

5. parameter address (가)이 없는가? \${not empty param.address } = ${ empty param.address }  

6. parameter id (가)이 없는가? \${not empty param.id } = ${ empty param.id } 
</pre>
